...
- **Per-turn weight**: The **final 4 turns** get 25% of the pool; **early turns** share the other 75%
  - Early: `(0.75 × totalPool) / max(1, totalTurns - 4)` per turn
  - Final 4: `(0.25 × totalPool) / 4` per turn
...
2. **Appeal (per demographic group)** — Quadratic position (50 − |econDiff|×5 − |socialDiff|×5)²/100 + `normalizeNPI(influence)`×25. Max 50 (position 25 + influence 25 at PI=100). Does NOT include favorability — that scales at the end.
...
5. **Party org scalar** — 0.5 + (state party organization/100)×0.5: higher party org in that state = better mobilization. Independents use 1.0.
...
| Factor                                 | Effect                                                                                                                                            |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ---- | ---------- | -------------------------------- |
| **Policy alignment**                   | Quadratic: (50 −                                                                                                                                  | econDiff | ×5 − | socialDiff | ×5)²/100; closer = higher appeal |
...
| **Favorability (approval)**            | Final scalar: favorability/100; 0% approval = 0 votes                                                                                             |
| **Party org**                          | Final scalar: 0.5 + (org/100)×0.5; higher state party org = better mobilization                                                                   |
...
- **Presidential vote weights**: Appeal uses **party-weighted positions** (3×party + candidate)/4 so party matters more. **Party org** uses very steep curve (0.05×–1.0×). **State lean** uses **party position**; red states boost R, blue boost D. Lean strength 1.1–1.2; state lean normalized by /4 (stronger); misaligned floor 0.15×.
...
| `voteCalculations.ts`    | Turn vote weight formula (75% early turns / 25% final 4 turns), state turnout calculation                                                          |
...
4. **Compute turn pool** — `turnVoteWeight()` allocates 75% of pool to early turns, 25% to final 4 turns
