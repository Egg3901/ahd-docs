...
For **AI-specific** investigation, validation, and handoff conventions, see [ai-development-workflow.md](./ai-development-workflow.md). For codebase structure, see [repo-operating-map.md](./repo-operating-map.md). The authoritative rules for code and git live in `CLAUDE.md` (in the a-house-divided app repo root) at the repo root.
...
- `MONGODB_URI` — e.g. `mongodb://localhost:27017/a-house-divided`
...
Seed a playable dataset:

```bash
npm run seed
```

For a fuller local dataset in one shot (runs several seed scripts in order):
...
| `verify`                                   | `lint` + `format:check` + `typecheck` + `architecture:audit`, then `test:run` - one-shot pre-push gate |
| `test:e2e`                                 | Playwright (`playwright.config.ts`); local use assumes a running app (see §6)                          |
| `seed`, `seed:reset`, `seed:all`, `seed:*` | Database seeding and domain-specific seeds                                                             |
...
| Security audit   | `npm audit --audit-level=high` (not bundled in `verify`) |
| Tests + coverage | `npm run test:coverage`                                  |

**`verify`** matches the local quality gate: lint, format check, typecheck, architecture audit, then `test:run`. It does **not** add coverage or `npm audit`. Run `test:coverage` before a risky change or when debugging coverage gaps.
...
1. `git fetch` and branch from **`development`** (see `CLAUDE.md` (in the a-house-divided app repo root) — feature → development → master).
...
1. Read `CLAUDE.md` (in the a-house-divided app repo root) and [repo-operating-map.md](./repo-operating-map.md).
2. Follow [ai-development-workflow.md](./ai-development-workflow.md) for investigation and validation structure.
3. Use `npm run verify` as the standard done check; use `npm run build` when changing anything that could affect the production bundle, Next.js routing, server/client boundaries, or build-time imports.
...
| Contributing text said “branch off `main`” while `claude.md` uses **development** / **master** | README Contributing updated; this doc repeats the three-branch model |
| Full CI uses coverage; local default was `test:run` only                                       | Use `npm run test:coverage` when you need CI parity                  |
