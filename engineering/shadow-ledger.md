...
A `gameConfig` feature flag (flags-as-seed-defaults, PR #2627 style): **off in prod
seeds, on in sim**. When on, `emitFinancialTx` also derives a balanced ledger entry from
each tx-log row, and the reconciler runs each turn. When off, none of the collections
below are written — a `trace_ledger` call on such a world correctly reports "no data".
