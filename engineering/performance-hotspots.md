...
- **News page:** `src/app/news/page.tsx` uses client-side `fetch("/api/character/me")`; auth-heavy layouts elsewhere may duplicate `/api/auth/me` patterns — consolidate only where product allows, to avoid double requests on first paint.
- **Large route surfaces:** 400+ API routes under `src/app/api/` — performance work is most valuable on high-QPS or turn-adjacent routes, not one-off admin tools.
